...
use crate::{
    ...
    types::{Assertion, AssertionExpression},
};
...
pub struct AssertionState {
    ...
    pub expressions: Vec<AssertionExpression>,
}

impl AssertionState {
    pub fn new(assertions: Vec<Assertion>, expressions: Vec<AssertionExpression>) -> Result<Self> {
        ...
        let btree = expressions
            ...
            .flat_map(|expression| match expression {
                AssertionExpression::And(assertion_indexes) => assertion_indexes.clone(),
                AssertionExpression::Or(assertion_indexes) => assertion_indexes.clone(),
            })
            .collect::<BTreeSet<u8>>();
        ...
        if !missing_indexes.is_empty() {
            expressions.push(AssertionExpression::And(missing_indexes));
        }
    ...
    pub fn evaluate(&self) -> Result<()> {
        for (_, expression) in self.expressions.iter().enumerate() {
            match expression {
                AssertionExpression::And(assertion_indexes) => {
                    let mut result = true;
                ...
                AssertionExpression::Or(assertion_indexes) => {
