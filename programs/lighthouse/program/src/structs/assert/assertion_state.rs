...
impl AssertionState {
    pub fn new(assertions: Vec<Assertion>, expressions: Option<Vec<Expression>>) -> Result<Self> {
        ...
        if let Some(expressions) = expressions {
            ...
            for (i, _) in assertion_results.iter().enumerate() {
                expressions.push(Expression::And(vec![i as u8]));
            }
